...
* Did you take Krista's webinar on Tuesday?  [yes / no]
* Have you used tidyverse before?  [yes / no]
* Have you used ggplot before?  [yes / no]

## Misc

The best reference for this material is Hadley Wickham's [R for data scientists](http://r4ds.had.co.nz/). This is adapted from a tutorial Mike Frank and co. put together for an ICIS 2019 tutorial! 
...
```{r}
round(mean(penguins$bill_length_mm, na.rm = T))
```
...
to_inches <- function (mm) { mm / 25.4 }

# first compute mean height in inches the normal, nested way  
round(mean(to_inches(penguins$bill_length_mm), na.rm=TRUE), digits = 2)
...
sgf_means <- sgf_subject_means %>%
  ...
  summarise(se = sd(mean_correct) / sqrt(n()), 
...
sgf_means |> 
  ggplot(aes(x = age_group, y = mean_correct, color = condition)) +
  geom_line(aes(group = condition)) +
  geom_linerange(aes(ymin = mean_correct-se, ymax = mean_correct+se)) +
  ylim(0,1) +
  theme_minimal() +
  labs(x = "Age",
       y = "Prop. Correct",
...
```{r}
# now make pretty ggplot of sgf by age and condition
ggplot(sgf_means, aes(x = age_group, y = mean_correct, 
...
# first get subject means
sgf_subject_means <- sgf %>%
  group_by(subid, condition, item, age_group) %>%
  summarise(mean_correct = mean(correct))

# then get group means
sgf_means <- sgf_subject_means %>%
  group_by(condition, item, age_group) %>%
  summarise(se = sd(mean_correct) / sqrt(n()), 
            mean_correct = mean(mean_correct), 
            n = n())
